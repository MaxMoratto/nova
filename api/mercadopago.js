...
// 1) Crea una orden en Firestore y reserva los asientos (VIP mesa / VIP asiento / Preferente) 10 min (anti doble-venta).
  ...
  try {
    ...
    try {
      await firestore.runTransaction(async (tx) => {
        const refs = seatIds.map(id => firestore.collection('asientos_nova').doc(id));
        const snaps = await Promise.all(refs.map(r => tx.get(r)));
        snaps.forEach((s, i) => {
          const d = s.data();
          if (d && (d.status === 'vendido' || (d.status === 'reservado' && d.expira > now && d.orderId !== orderId)))
            throw new Error('El asiento ' + seatIds[i] + ' ya no está disponible');
        });
    ...
    if (!r.ok) {
      ...
      try {
        ...
        seatIds.forEach(id => batch.delete(firestore.collection('asientos_nova').doc(id)));
