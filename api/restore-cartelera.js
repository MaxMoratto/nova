// TEMPORAL: REVIERTE estelar/co-estelar a las fotos previas (respaldo).
// Uso:  /api/restore-cartelera?key=nova-restore-2026
// BORRAR este archivo al terminar.
const { db } = require('../lib/firebase');

const MAP = {
  principal: { rojo: 'backups/prev-principal-r.webp', azul: 'backups/prev-principal-a.webp' },
  coestelar: { rojo: 'backups/prev-coestelar-r.webp', azul: 'backups/prev-coestelar-a.webp' }
};
...
module.exports = async (req, res) => {
  if ((req.query.key || '') !== 'nova-restore-2026') { res.status(403).send('no autorizado'); return; }
  let firestore;
  ...
  try {
    ...
    res.status(200).json({ ok: true, revertidas: done });
